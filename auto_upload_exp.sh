...
QUAL=`${HOSTNAME} -s`
QUEM=`${WHOAMI}`
SAP_SCP="/usr/local/bin/scp_sap.expect"
...
function preparation_(){
        ...
        if [ ! -x ${EXP} ] ; then
                echo "3:500:ERROR - command expect not found."    # returncode = 3 = System Error - put sensor in DOWN status
                do_log_ ERROR - command expect not found.
                die_ ;
        fi
#       is_root_;
        is_usr_;
